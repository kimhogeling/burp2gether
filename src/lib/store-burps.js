...
export const supportedAudioMimeType = audioOptions.find(MediaRecorder.isTypeSupported) || null;

const createISOStringWithTimezoneOffset = date =>
...
export const burps = readable(null, set => {
    onSnapshot(query(collection(getFirestore(), "burp"), orderBy("date", "desc"), limit(100)), ({docs}) => {
...
export function addReactionToFirebase(reaction, burp, authUser) {
    ...
    if (!updatedReactions[reaction.key].includes(authUser.uid)) {
        updatedReactions[reaction.key] = [authUser.uid, ...updatedReactions[reaction.key]];
    } else {
        updatedReactions[reaction.key] = updatedReactions[reaction.key].filter(uid => uid !== authUser.uid);
    }
